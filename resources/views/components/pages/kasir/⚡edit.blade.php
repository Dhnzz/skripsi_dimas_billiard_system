...
new #[Layout('layouts.app', ['title' => 'Edit Kasir', 'breadcrumbs' => [['title' => 'Manajemen Kasir', 'url' => '/owner/kasir'], ['title' => 'Edit Kasir', 'url' => '#']]])] class extends Component {
    use WithFileUploads;
    ...
    public function mount($id)
    {
        ...
        $this->userId = $kasir->id;
        $this->name = $kasir->name;
        $this->email = $kasir->email;
        $this->phone = $kasir->phone;
        $this->is_active = $kasir->is_active;
        $this->existingAvatar = $kasir->avatar;
    }
    ...
    public function save()
    {
        ...
        $kasir->name = $this->name;
        $kasir->email = $this->email;
        $kasir->phone = $this->phone;
        $kasir->is_active = $this->is_active;

        // Update password hanya jika diisi
        if (!empty($this->password)) {
            $kasir->password = Hash::make($this->password);
        }
        ...
        if ($this->avatar) {
            ...
            $kasir->avatar = $this->avatar->store('avatars', 'public');
        } elseif ($this->existingAvatar === null && $kasir->avatar) {
            // User menghapus avatar tanpa mengunggah yang baru
            if (Storage::disk('public')->exists($kasir->avatar)) {
                Storage::disk('public')->delete($kasir->avatar);
            }
            $kasir->avatar = null;
        }
        ...
        session()->flash('success', 'Data kasir berhasil diperbarui!');

        return $this->redirectRoute('owner.kasir.index', navigate: true);
    }
};
?>
        ...
        <div class="row">
                        ...
                        <div class="form-group">
                            <div class="profile-img-edit position-relative d-flex justify-content-center mb-3">
                                ...
                                @elseif ($existingAvatar)
                                    <img src="{{ Storage::url($existingAvatar) }}" alt="profile-pic"
                                        class="theme-color-default-img profile-pic rounded"
                                        style="object-fit: cover; width: 150px; height: 150px;">
                                @else
            ...
            <div class="col-xl-9 col-lg-8">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">Edit Informasi Kasir</h4>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="new-user-info">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                                        id="name" wire:model="name" placeholder="Nama lengkap kasir">
                                    @error('name')
                            ...
                            <div class="d-flex justify-content-end">
                                <a href="{{ route('owner.kasir.index') }}" wire:navigate
